import { getConversionReport } from "@/lib/admin/reports"
...
export const metadata = { title: "Admin — Conversion Reports" }
...
export default async function AdminConversionReportsPage({ searchParams }: Props) {
  const { from: fromStr, to: toStr } = await searchParams
  const { from, to } = resolveDateRange(fromStr, toStr)

  const report = await getConversionReport(from, to)
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-border-default overflow-hidden">
        <div className="flex items-center justify-between p-5 pb-0">
          ...
          <a
            href={`/api/admin/reports/export?type=conversion&${exportQuery}`}
