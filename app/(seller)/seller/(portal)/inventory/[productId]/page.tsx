...
import { sellerAdjustStock } from "@/lib/inventory/actions"
...
export default async function SellerProductInventoryPage({ params, searchParams }: Props) {
  ...
  const data = await getProductWithMovements(productId, page, { shopId: shop.id })
  if (!data) notFound()
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/seller/inventory" className="text-xs text-brand-600 hover:underline">
          ← Back to Inventory
        </Link>
        ...
        <form className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2">
            ...
            <input
              ...
              required
              className="w-full text-sm border border-border-default rounded px-3 py-2"
              placeholder="e.g. Damaged, Recount, New shipment"
            />
          </div>
          <div className="col-span-2">
            <button
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-4 mt-4 border-t border-border-default">
            ...
            <div className="flex gap-2">
              {page > 1 && (
                <Link
                  href={`/seller/inventory/${product.id}?page=${page - 1}`}
